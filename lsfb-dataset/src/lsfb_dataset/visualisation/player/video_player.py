...
class VideoPlayer:
    ...
    def __init__(self, root: Optional[str] = None, screenshot_dir: Optional[str] = None):
    ...
    def _add_landmarks(self, landmarks: np.ndarray, landmarks_type: str, mesh: bool = False):
        if self.skeleton is None:
            self.skeleton = Skeleton(mesh=mesh, resolution=self.resolution)

        self.skeleton.add_landmarks(landmarks, landmarks_type)

        landmarks_count = self.skeleton.landmarks_count
        if self.video is not None:
