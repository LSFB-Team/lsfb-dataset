...
class LSFBContBase:
    ...
    def _load_annotations(self):
        if self.config.segment_level == "subtitles":
            raise NotImplementedError(
                "Subtitles are not yet available nor implemented."
            )
            # TODO: add subtitles
        ...
        with open(
            f"{self.config.root}/annotations/{prefix}_{suffix}.json", "r"
        ) as file:
    ...
    def _make_windows(self):
        ...
        self.windows = []
        for instance_id, n_frames in self.instance_metadata[
            ["id", "n_frames"]
