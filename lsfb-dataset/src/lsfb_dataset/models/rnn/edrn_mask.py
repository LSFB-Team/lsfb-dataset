...
class EDRNCell(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, d: int):
        ...
        self.A_diag_mask = torch.zeros(M, MD)
        self.A_diag_mask.fill_diagonal_(1.0)
    ...
    def init_weights(self):
        ...
        with torch.no_grad():
            ...
            self.A_th.fill_diagonal_(0.0)
            self.A_ot.fill_diagonal_(0.0)

            init.uniform_(self.A_st, -stdv, stdv)
            init.uniform_(self.A_pt, -stdv, stdv)
            self.A_pt = Parameter(torch.masked_fill(self.A_pt, self.A_pt_mask, value=0.0))
